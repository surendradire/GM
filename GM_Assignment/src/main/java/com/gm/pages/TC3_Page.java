...
public class TC3_Page {
	...
	public boolean imageVeification() throws InterruptedException {
		...
		new WebDriverWait(driver, 40).until(ExpectedConditions.visibilityOf(firstImage));

		firstImage.click();

		new WebDriverWait(driver, 40).until(ExpectedConditions.visibilityOf(ObjectgegevensLbl));
		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", ObjectgegevensLbl);

		Thread.sleep(4000);
		new WebDriverWait(driver, 60).until(ExpectedConditions.visibilityOf(ObjectgegevensLbl));
		ObjectgegevensLbl.click();

		new WebDriverWait(driver, 40).until(ExpectedConditions.visibilityOf(JH_nummer));
